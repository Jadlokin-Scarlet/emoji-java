...
public class EmojiParser {
  ...
  /**
   * if the substring [startPos, resultInt) is emoji, return resultInt, else -1.
   * This returns the longest matching emoji, for example, in
   * "\uD83D\uDC68\u200D\uD83D\uDC69\u200D\uD83D\uDC66"
   * it will find alias:family_man_woman_boy, NOT alias:man
   * @see #getEmojiEndPos(char[], int)
   */
  public static int getEmojiEndPosQuick(char[] text, int startPos) {
    int emojiEndPos = EmojiManager.EMOJI_TRIE.getEmojiEndPos(text, startPos);
    if (emojiEndPos > 0 && emojiEndPos < text.length
            && isVariationSelector(text[emojiEndPos])) {
      emojiEndPos++;
    }
    return emojiEndPos;
  }
